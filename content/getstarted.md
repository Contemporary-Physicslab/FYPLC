...
Pixi might not work correctly when the project files are stored in OneDrive. To check if this is the case, execute `pwd` in the terminal and check if the path contains the word "OneDrive". Move the project files to a local folder, e.g. `C:\Users\<my_username>\Documents`.
